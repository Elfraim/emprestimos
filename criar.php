...
$query = "INSERT INTO emprestimo (nome_completo, ra, funcao,equipamento,quantidade,data_emprestimo) VALUES('$nome','$ra', '$funcao','$equip','$quant','$data');";
...
">
  ...
  <li class="nav-item" style="zoom: 104%;background-color: #f3f3f3;box-shadow: 0 0 2px #000000;
">
 ...
 while($user_data = mysqli_fetch_assoc($result))
{
  ...
  echo '  <li class="list-group-item" name="lista" onclick="showDetails(this)" id="owl" value="'.$ra.'" data-nome="'.$nome.'" data-func="'.$funcao.'">Nome: '.$nome.'&nbsp <p style="float: right"> '.$funcao.'</li></p> ';
...
<ul>
  ...
  <input  style="display: contents" class="form-control"name="nome" id="nome" type="text" >
<br>
  
  <input  style="display: contents" class="form-control" name="funcao" id="funcao" type="text">
<br>
 
  <input  style="display: contents" class="form-control" name="ra" id="ra" type="number">
</ul>

<label for="exampleFormControlInput1">Equipamento:</label>
<select name="equipamento" class="form-select form-select-sm" aria-label=".form-select-sm example">
...
echo '<option name="equipamento" value="'.$u['id'].'"required="" class="form-control" id="exampleFormControlInput1" placeholder="Ex: Notebook">'.$u['equipamento'].'</option>';
